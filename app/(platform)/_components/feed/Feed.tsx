...
export default function Feed() {
  ...
  useEffect(() => {
    const fetchAcademias = async () => {
      try {
        const res = await fetch(`/api/academia?page=${page}&limit=${limit}&param=${searchParam}`);
  ...
  return (
    <div className="container mx-auto p-2 bg-mercury">
      <div className="flex flex-col gap-4 p-4">
        {isSearching ? <Spinner /> : academias.map((academia) => (
      ...
      <div className="flex justify-end items-center mt-6 gap-2 p-4">
        ...
        <button
          onClick={() =>{ setPage((prev) => prev + 1); console.log(totalPages)}}
